...
enum { LVAL_ERR, LVAL_NUM, LVAL_SYM, LVAL_STR, LVAL_FUN, LVAL_SEXPR, LVAL_QEXPR, LVAL_EXIT };
...
lval* lval_copy(lval* v) {
    ...
    switch (v->type) {
        ...
        case LVAL_ERR: x->err = malloc(strlen(v->err) + 1);
            strcpy(x->err, v->err);
        break;
        case LVAL_SYM: x->sym = malloc(sizeof(v->sym) + 1);
            strcpy(x->sym, v->sym);
        break;
...
lval* lval_call(lenv* e, lval* f, lval* a) {
    ...
    while (a->count) {
        ...
        if (f->formals->count == 0) {
            lval_del(a);
            return lval_err("Functions passed too many arguments. "
...
int main(int argc, char** argv) {
    ...
    mpca_lang(MPCA_LANG_DEFAULT,
        "                                                                                \
            ...
            sexpr   : '(' <expr>* ')' ;                                                  \
            qexpr   : '{' <expr>* '}' ;                                                  \
            expr    : <number> | <symbol> | <string> | <sexpr> | <comment> | <qexpr> ;   \
            lispy   : /^/ <expr>* /$/ ;                                                  \
        ",
